...
class ApplePodcastsAdapter(BaseAdapter):
    ...
    def _fetch_listen_history(
        ...
        try:
            ...
            if item_count > 0 and error_count == item_count:
                logger.error(f"All {item_count} items from /podcasts/listen-history failed validation; 100% malformed")
    ...
    def _fetch_transcripts(
        ...
        try:
            ...
            # Only raise error if all items were malformed (raised exceptions)
            if item_count > 0 and error_count == item_count:
                logger.error(f"All {item_count} items from /podcasts/transcripts failed validation; 100% malformed")
