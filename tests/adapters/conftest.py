...
def mock_podcasts_httpx_get(monkeypatch):
    ...
    monkeypatch.setattr(
        "context_library.adapters.apple_podcasts.httpx.get",
        mock_get
    )
...
@pytest.fixture
def mock_all_podcasts_endpoints(mock_podcasts_httpx_get):
    """Fixture that configures all Apple Podcasts endpoints with empty responses.
    ...
    for endpoint in endpoints:
        mock_podcasts_httpx_get.set_response(f"{base_url}{endpoint}", [])

    return mock_podcasts_httpx_get


@pytest.fixture
def mock_screentime_httpx_get(monkeypatch):
    """Fixture for mocking httpx.get() function for Apple ScreenTime endpoints with request tracking.
    ...
    monkeypatch.setattr(
        "context_library.adapters.apple_screentime.httpx.get",
        mock_get
    )
...
@pytest.fixture
def mock_all_screentime_endpoints(mock_screentime_httpx_get):
    """Fixture that configures all Apple ScreenTime endpoints with empty responses.
