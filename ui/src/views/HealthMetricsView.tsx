 ...
 * and removing _summary/_series/_measurement suffixes.
 */
function humanizeHealthType(healthType: string): string {
  const label = healthType
    .replace(/_summary$|_series$|_measurement$/, '') // Remove suffixes
    .split('_')
 ...
 * Falls back to UnknownHealthTypeCard for any unrecognized types,
 * which gracefully displays metadata for future type support.
 */
function HealthMetricCard({ chunk, healthType }: { chunk: ChunkResponse; healthType: string }): ReactNode {
  switch (healthType) {
    ...
    // Fallback for unknown types (including any future BA spec types)
    default:
